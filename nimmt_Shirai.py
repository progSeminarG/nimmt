...
class ShiraiAI(Player):
    def __init__(self):
        ...
        self.para=[6.0, 2.0, 0.0, -6.0, 4, 10, 0]
    ...
    def cal1(self):
        ...
        for i in range(len(self.my_cards)):#修正平井式初期値設定2.5
            if self.my_cards[i]>=75 and i >= 0.5*len(self.my_cards):
                eval_list[i]=0*i
            else:
                eval_list[i]=0.5*i
        #print("ini--",eval_list)
        ...
        #定数型危険察知
        for i in range(4):
            danger[i]=max(self.dealer.field[i])+6*(6-len(self.dealer.field[i]))
            for j in range(len(eval_list)):
                if danger[i]-self.para[0] < self.my_cards[j] < danger[i]+self.para[0]: #[0]=7
                    eval_list[j]-=10#PARA
        ...
        for k in range(len(self.my_cards)):
            ...
            if count[k]<=4-len(self.dealer.field[min_difcol]):
                ...
                eval_list[k]+=-count[k]/self.para[1]+self.para[2]#[6]=2[7]=0
        #print("count-",count)
        #戦略的自滅
        #print("caw",caw)
        if max(eval_list)<=self.para[3] and min(caw)<=self.para[4] and len(self.my_cards)>1:#[8]=-6[9]=4
            risk=0
            for card in self.remains:
                if card<min(self.my_cards):
                    risk+=1
            if risk<=len(self.remains)/self.para[5]+self.para[6]:#[5]=10,[6]=0
                eval_list[0]+=100
